...
namespace flier268.Win32API
{
    public abstract class User32
    {
        ...
        /// <summary>
        /// Get mouse position in high resolution monitor
        /// </summary>
        /// <returns>mouse in all monitor (range: 0~65535)</returns>
        public static Point GetHighResolutionMousePosition()
        {
            _ = GetCursorPos(out var p);
            return GetHighResolutionMousePosition(new Point(p.x, p.y));
        }

        /// <summary>
        /// Get mouse position in high resolution monitor
        /// </summary>
        /// <param name="position">position get from API</param>
        /// <returns>mouse in all monitor (range: 0~65535)</returns>
        public static Point GetHighResolutionMousePosition(Point position)
        {
            _ = GetWindowRect((IntPtr)GetDesktopWindow(), out var rect);
            return GetHighResolutionMousePosition(position, rect);
        }
        ...
        /// </summary>
        /// <param name="position">position get from API</param>
        /// <param name="rect">WindowRect(can get it with GetWindowRect function)</param>
        /// <returns>mouse in all monitor (range: 0~65535)</returns>
        public static Point GetHighResolutionMousePosition(Point position, RECT rect)
        {
            var x = (int)((double)position.X / rect.Right * 65536.0);
            var y = (int)((double)position.Y / rect.Bottom * 65536.0);
